...
async def hide_show_profile(message:Union[types.Message, types.CallbackQuery]):
    ...
    if status_user == 'user':
        text = '''
Для того что б не удалять свою анкету вы можете скрыть ее, при этом не будет появляться у других пользователей в выдаче, 
но вы так же можете просматривать и лайкать анкеты других участников'''
    elif status_user == 'hidden_user':
        text = '''
Ваша анкета скрыта, нажмите "Сделать видимой" и ваша анкета появиться в выдаче другим пользователям'''
    else:
