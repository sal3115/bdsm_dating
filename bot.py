...
async def main():
    ...
    await runner.setup()
    site = web.TCPSite( runner, "0.0.0.0", 5602 )

    async def stop_services():
        """Корректное завершение всех сервисов."""
        logger.info( "Stopping services..." )
        await runner.cleanup()  # Остановка вебхука
        await dp.storage.close()  # Закрытие хранилища FSM
        await dp.storage.wait_closed()
        await bot.session.close()
    ...
    try:
        ...
        logger.info( "Shutting down bot and webhook..." )
    finally:
        await stop_services()
